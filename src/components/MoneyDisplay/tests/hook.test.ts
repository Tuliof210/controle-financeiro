...
describe("useMoneyDisplay", () => {
  it("defaults to base, no sign colour, dimming off", () => {
    ...
    expect(result.current.className).toContain("base");
    expect(result.current.className).not.toContain("positive");
    expect(result.current.className).not.toContain("negative");
  ...
  it("dims hero and large unless told otherwise", () => {
    ...
    expect(hero.result.current.className).toContain("hero");
  });
  ...
  it("keeps the Unicode minus on the head", () => {
    ...
    expect(result.current.label).toBe("−R$ 640,00");
  });
  ...
  it("colours by sign only when asked", () => {
    ...
    expect(result.current.className).toContain("negative");
  });
});
