...
// The first point has no run behind it, so the solid line starts empty rather
// than as a one-point path.
function solidRun<T>(points: T[], index: number): T[] {
  if (index === 0) {
    return [];
  }
