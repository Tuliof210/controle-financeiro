...
export type LimitCardProps = {
  ...
  // The range's current month: everything past it is a projection.
  current: number;
};

export function useLimitCard({ limit, current }: LimitCardProps) {
  const rows = limit.months.map((month) => {
