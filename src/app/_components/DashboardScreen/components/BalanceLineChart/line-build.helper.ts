...
// The frame, the scales, the dash split and every dot, in one pass. Split off
// `hook.ts` so that file can be the memo boundary and nothing else — the whole
// point is that this runs when points/width/height change, not on every pointer
// move.
...
// Hands the plot width to the mark, which hands it to the tag. The tag places
// itself: only it knows how wide its own label is.
const markFor = (
  ...
  return { x: x(point), y: y(point), width: innerWidth };
};
