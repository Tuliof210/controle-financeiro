...
const teto = (
  ...
  ceilingLeft: number,
  budget = 50,
): CeilingMonth => ({ month, budget, ceilingBalance: 0, ceilingLeft });

const points = [point(202_601, 100), point(202_602, 200), point(202_603, 300)];
const scale = {
  xOf: (month: number) => month,
  yOf: (value: number) => value,
};
...
describe("dotsFor", () => {
  it("keeps one current dot per month when there is no teto series", () => {
    const dots = dotsFor(points, [], Number.POSITIVE_INFINITY, scale);
  ...
  it("puts both dots on the same y when ceilingLeft equals cumulative", () => {
    const dots = dotsFor(points, [teto(202_602, 200, 0)], 0, scale);
