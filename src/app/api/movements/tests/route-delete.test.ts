...
import { DELETE } from "@/app/api/movements/route.ts";
...
const remove = jest.mocked(deleteMovement);

const del = (query: string) =>
  DELETE(
    new NextRequest(`http://x/api/movements${query}`, { method: "DELETE" }),
  );
...
describe("DELETE", () => {
  ...
  it("rethrows anything else", async () => {
    ...
    await expect(del("?id=m1")).rejects.toThrow();
  });
});
