...
- [playwright] `playwright.config.ts` is re-imported in every worker, after `webServer` ran — a module-scope side effect (a DB delete, a seed) fires mid-run, and only once specs exist, so a zero-spec run hides it; put setup in `webServer.command` (2026-07-28)
