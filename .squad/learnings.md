...
- [browser-preview] `preview_start {name}` launches from the main checkout, never a worktree (run the server there, then `preview_start {url}`); the pane is shared, a backgrounded tab reports `innerWidth: 0`, `computer`'s `key` misses focused inputs (`triple_click`+`type`), `screenshot` returns the pre-scroll frame, and reading state in the SAME `javascript_tool` call as the change returns the PRE-change value (2026-07-29)
...
- [next/dev] Next 16 dev refuses a second dev server from the same directory even on a free port — kill a manual measurement server before `npm run test` boots its own on :3100 (2026-07-28)
...
- [ps-check] the DEBT sweep cuts each entry at the first `:` and only checks the FILE exists, so one naming a deleted SYMBOL in a surviving file reports `0 to sweep` while pointing at nothing — grep debt.md for the symbols a story deletes (2026-07-30)