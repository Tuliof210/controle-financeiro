...
- [worktree] A fresh worktree needs three manual setup steps, all because `git worktree add` skips gitignored paths: skip `ps-check.sh warm`'s `node_modules` symlink and run a real `npm install` (the symlink breaks Turbopack), `npx prisma generate` (`src/generated/` is absent, so every route 500s on `@/generated/prisma/client` and Turbopack caches that failure — restart it after), and copy `dev.db` from the main checkout before migrating (a `0` row count means a missing copy, not lost data) (2026-07-29)
...
- [playwright] A native `<dialog>` defeats two habits: a second one anywhere (e.g. a nav drawer) makes `getByRole("dialog")` a strict-mode violation, and a modal is `position: fixed` in the top layer so it adds NOTHING to `documentElement.scrollWidth` — an overflow assertion for modal content silently cannot fail. Scope by `page.locator("dialog[open]")` and measure its own `scrollWidth - clientWidth`, proving it reddens by injecting a `min-width` (2026-07-29)
