...
- [playwright] Wrapping a cell does not break `getByText(name, { exact: true })` — the text engine drops an element when a child matches the same text — but a regex like `/^R\$/` matches the wrapper too once the wrapper's own text starts there (2026-07-28)
