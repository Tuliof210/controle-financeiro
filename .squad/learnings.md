...
- [design-system] The hit-target rule (>=44px) binds every new interactive element, not just ones reusing `IconButton` — hardcode 44px (`<summary>` wants `line-height`; `display:flex` drops its marker), or an overlaid `::after` when padding would shift layout (added 2026-07-25, extended 2026-07-27)
