...
- [publish] Leave the plan commit on the story branch — `/ps:run` is serial now so nothing collides on `story.md`, and a copy committed to main is inside the squash anyway, so `git pull --rebase` after the merge conflicts on it and needs `--skip` (2026-07-28)
- [worktree] `node_modules` is per-checkout and cannot be shared: `ps-check.sh warm`'s symlink makes Turbopack refuse every build ("Symlink node_modules is invalid, it points out of the filesystem root"), so delete both links and run a real `npm install`, then `npm run db:setup` — `db:postinstall` is a named script, not npm's `postinstall` hook, so a fresh install leaves `@/generated/prisma` missing and the build fails on a source line (2026-07-28)
