...
- [prisma/migrate] EVERY SQLite schema change here comes out as a table rebuild, additive columns with a literal default included — `migrate dev --create-only`, then read the SQL before applying: the `PRAGMA foreign_keys=OFF` pair is what stops `DROP TABLE` cascading into children, and a column absent from the `INSERT` list is backfilled by the new table's DEFAULT. Hand-edit only to backfill; `prisma generate` after (2026-07-29)
