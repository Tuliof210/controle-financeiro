...
- [prisma/migrate] `migrate dev --create-only` needs hand-edited SQL for any data-preserving change: a column drop is a table rebuild (backfill between the CREATE and the rebuild), and a simultaneous multi-model rename plus a renamed FK column defaults to `DROP TABLE`+`CREATE` instead of `ALTER TABLE ... RENAME` — read the generated SQL before applying; run `prisma generate` after (2026-07-29)
