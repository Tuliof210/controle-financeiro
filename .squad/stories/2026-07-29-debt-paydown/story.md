...
## Acceptance Criteria
- [ ] Re-importing an OFX already on file leaves Importar disabled with its tooltip, on the server's answer alone
- [ ] No viewport width makes an entry row narrower by growing the window
- [ ] A goal row's name and its dropped amount start at the same x below the one-line floor
- [ ] The ceiling card names how many months remain, not "do período"
- [ ] RowGrid's one-line floor exists once as a Sass symbol; nothing hand-copies it
- [ ] No tracked file exceeds the 100-line cap, and no doc statement about `e2e/` is false

## Definition of Done
- [ ] `npm run lint` exits 0
- [ ] `npx tsc --noEmit` exits 0 and silent
- [ ] `npm run test:e2e` — 53+ tests, all green
- [ ] `sh .claude/ps-check.sh` reports `needs attention: 0`
- [ ] `.squad/debt.md` retains only the three entries this story does not close

## Tasks
- [ ] tasks/01-api-error-code.md — carry `error.code` through `api.ts`; ImportAction disables on a 409
