...
## Acceptance Criteria
- [x] Each interval row on /previsoes' forecast form carries its own "mês único"
      control; two rows in the same form can be in different states.
- [x] With it on, the row shows exactly one MonthPicker, spanning the width the
      pair used to take; picking a month sets both ends of that interval.
- [x] With it off, the row shows the Início/Fim pair unchanged, and the Fim value
      differs from Início so the row stays off until the user says otherwise.
- [x] Reopening a saved forecast shows a one-month interval already locked.
- [x] Saving from a locked row stores that single month — the request body, the
