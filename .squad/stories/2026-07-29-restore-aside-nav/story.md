...
## Acceptance Criteria
- [ ] No bottom navigation bar exists at any viewport width, mobile included —
      the component is gone from the codebase, not hidden by CSS.
- [ ] From 768px up the rail sits in the layout expanded, and the header
      hamburger still collapses it to the icon strip and back.
- [ ] Below 768px the same rail opens as an overlay drawer over the content
      from the header hamburger, and closes on Esc, on a backdrop click and on
      navigating to another screen.
- [ ] The header shows the hamburger at every width; its brand mark is gone
      (the rail head already carries the brand).
- [ ] Every destination in the nav is reachable at 375px and at 1440px.
- [ ] No horizontal overflow at 375 / 767 / 768 / 1024 / 1440px.

## Definition of Done
- [ ] `npm run lint` clean and `npm run test` green (both tasks).
- [ ] `npm run dev` + browser preview at 375px and 1440px: drawer opens and
...
- [ ] tasks/02-mobile-nav-drawer.md — below `md` the rail opens as an overlay
